...
# BANQUE C (Notes MIDI 32-47) : Pads Aléatoires (Synthèse de Test)
# IMPORTANT : Remplacez les chemins d'accès (ex: "path/to/kick.wav")
# par le chemin réel de vos fichiers .wav sur votre machine.

BANK_C_SAMPLES = [
  [:sample, "path/to/kick.wav", 1, {release: 0.2, amp: 1.2}],                   # PAD 1 : Kick
  [:sample, "path/to/long_pad_32.wav", 1, {sustain: 2.0, release: 1.0, amp: 0.5}], # PAD 2 : Long Pad (anciennement prophet)
  [:sample, "path/to/hi_hat_closed.wav", 1, {attack: 0.01, release: 0.1, amp: 1.5}], # PAD 3 : Hi-Hat
  [:sample, "path/to/snare_clap.wav", 1, {release: 0.1, amp: 1.5}],            # PAD 4 : Snare/Clap
  [:sample, "path/to/fx_sweep.wav", 1, {release: 0.3, amp: 1.5}],              # PAD 5 : FX Sweep
  [:sample, "path/to/percussion_tri.wav", 1, {release: 0.1, amp: 1.3}],        # PAD 6 : Percu/Tri
  [:sample, "path/to/white_noise.wav", 1, {attack: 0.1, release: 0.5, amp: 1.5}],# PAD 7 : CNoise (Court)
  [:sample, "path/to/bass_dpulse.wav", 1, {release: 0.5, sustain: 0.5, amp: 1.0}],# PAD 8 : Bass DPulse
  [:sample, "path/to/lead_saw.wav", 1, {release: 0.1, amp: 1.5}],              # PAD 9 : Lead Saw
  [:sample, "path/to/bell_dull.wav", 1, {release: 0.5, amp: 1.5}],             # PAD 10 : Bell
  [:sample, "path/to/short_sine.wav", 1, {release: 0.05, attack: 0.001, amp: 1.5}],# PAD 11 : Short Sine
  [:sample, "path/to/fm_mid.wav", 1, {release: 0.5, amp: 1.5}],                # PAD 12 : FM Mid
  [:sample, "path/to/fm_low.wav", 1, {release: 0.5, amp: 1.5}],                 # PAD 13 : FM Low
  [:sample, "path/to/cnoise_short.wav", 1, {release: 0.01, amp: 1.5}],         # PAD 14 : CNoise (Très court)
  [:sample, "path/to/dull_bell_c4.wav", 1, {release: 0.8, amp: 1.5}],           # PAD 15 : Dull Bell C4
  [:sample, "path/to/growl_fx.wav", 1, {release: 0.2, sustain: 0.1, amp: 1.5}], # PAD 16 : Growl FX
]

# Exemple de fonction pour jouer le PAD sélectionné :
# Cette fonction doit être adaptée à la logique de votre contrôleur de PADs
define :play_pad do |pad_index|
  pad_config = BANK_C_SAMPLES[pad_index]
  
  # Vérification de base pour s'assurer que l'index est valide
  unless pad_config.nil? || pad_config[0] != :sample
    # On joue l'échantillon avec les paramètres (options)
    sample pad_config[1], pad_config[3] 
  else
    puts "Erreur: Le PAD #{pad_index} n'est pas configuré correctement."
  end
end

# Exemple d'utilisation (jouer le premier PAD)
# play_pad(0)
