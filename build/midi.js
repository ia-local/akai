...
const CC_MAPPING_ROLES = {
    // ⬅️ Knob 1 (physique) envoie CC 6 -> Rôle Volume Maître
    6:{ variable: 'master_amp', fn: (norm_v) => (1.0 - norm_v) * 1.5 },  
    
    // ⬅️ Knob 2 (physique) envoie CC 11 -> Rôle EQ Basses
    7: { variable: 'eq_bass',    fn: (norm_v) => 40 + norm_v * 80 },         
    
    // ⬅️ Knob 3 (physique) envoie CC 8 -> Rôle Vélocité
    2: { variable: 'note_velocity', fn: (norm_v) => 0.5 + norm_v * 0.7 },   
    
    // ⬅️ Knob 4 (physique) envoie CC 9 -> Rôle EQ Tonalité
    3: { variable: 'eq_band',   fn: (norm_v) => 40 + norm_v * 80 },         
    
    // ⬅️ Knob 5 (physique) envoie CC 10 -> Rôle Clé/Tonalité
    4: { variable: 'key_mode_index', fn: (norm_v) => Math.floor(norm_v * 12) }, 
    
    // ⬅️ Knob 6 (physique) envoie CC 7 -> Rôle EQ Aigus
